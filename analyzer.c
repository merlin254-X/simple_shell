...
/**
 * duplicateSubstring - Duplicate a portion of characters from a string.
 * 
 * @param sourceStr - The source string.
 * @param startIndex - The starting index of the substring to duplicate.
 * @param endIndex - The ending index of the substring to duplicate.
 * 
 * @return A dynamically allocated string containing the specified substring,
 ...
 *         if found, or NULL if the command is not in the specified PATH.
...
int isExecutableCommand(ShellInfo *shellInfo, char *filePath)
{
    ...
    {
        if (stat(filePath, &fileInfo) == 0 && (S_ISREG(fileInfo.st_mode) || S_ISLNK(fileInfo.st_mode)))
        {
